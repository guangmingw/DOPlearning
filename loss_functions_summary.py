...
def build_flow_not_intersect(maskp01, flows, tgt_img, ref_imgs, intrinsics, intrinsics_inv, depth, depth_ref, pose, weight = 1, PATCH = 3, batch_size = 4, rotation_mode='euler', padding_mode='zeros',):

    def compute_intersect_loss_weights(img, beta=1):

        patches_img1 = torch.nn.functional.unfold( img[:, 0, :, :].unsqueeze(1),  (PATCH, PATCH )).permute(0,2,1)# [B, (H-2)*(W-2), 9]
        patches_img2 = torch.nn.functional.unfold( img[:, 1, :, :].unsqueeze(1),  (PATCH, PATCH )).permute(0,2,1)# [B, (H-2)*(W-2), 9]
        patches_img3 = torch.nn.functional.unfold( img[:, 2, :, :].unsqueeze(1),  (PATCH, PATCH )).permute(0,2,1)# [B, (H-2)*(W-2), 9]

        patches_img_middle1 = patches_img1[:, :, int((PATCH * PATCH - 1) / 2)].unsqueeze(2) 
        patches_img_middle2 = patches_img2[:, :, int((PATCH * PATCH - 1) / 2)].unsqueeze(2) 
        patches_img_middle3 = patches_img3[:, :, int((PATCH * PATCH - 1) / 2)].unsqueeze(2)
 
        patches_img_deta = (torch.abs(patches_img1 - patches_img_middle1) + torch.abs(patches_img2 - patches_img_middle2) + torch.abs(patches_img3 - patches_img_middle3))/3 # [B, (H-2)*(W-2), 9]

        return torch.exp(-(beta * torch.abs(patches_img_deta)))  #[B, (H-2)*(W-2), 9]

    def one_scale(depth, depth_ref, flows, mask_p01): #flows [flows_fwd[i], flows_bwd[i]]
        ...
        ref_img_warped_bwd = flow_warp(ref_imgs_scaled[1], flows[1])

        # fwd_mask_p01_pyramid = mask_p01[:,0,:,:].unsqueeze(1)
        # bwd_mask_p01_pyramid = mask_p01[:,1,:,:].unsqueeze(1)


        flows_fwd_x = flows[1][:, 0, :, :].unsqueeze(1)# [B,1,H,W]
        flows_fwd_y = flows[1][:, 1, :, :].unsqueeze(1)
        ...
        #[B, (H-2)*(W-2), 9] 
        #===========================================================================================================
        flows_bwd_x = flows[0][:, 0, :, :].unsqueeze(1)# [B,1,H,W]
        flows_bwd_y = flows[0][:, 1, :, :].unsqueeze(1)
        flows_bwd_x_patches = torch.nn.functional.unfold(flows_bwd_x,(PATCH, PATCH)).permute(0,2,1)# [B, (H-2)*(W-2), 9]
        flows_bwd_y_patches = torch.nn.functional.unfold(flows_bwd_y,(PATCH, PATCH)).permute(0,2,1)

        flows_bwd_x_patches_middle = flows_bwd_x_patches[:,:, int((PATCH * PATCH - 1) / 2)].unsqueeze(2)# [B, (H-2)*(W-2), 1]
        flows_bwd_y_patches_middle = flows_bwd_y_patches[:,:, int((PATCH * PATCH - 1) / 2)].unsqueeze(2)

        bwd_full_flow_deta_old = -flows_bwd_x_patches_middle * flows_bwd_y_patches + flows_bwd_x_patches * flows_bwd_y_patches_middle
        ...
        mask_bwd_full_flow_deta = torch.where(torch.abs(bwd_full_flow_deta_old) > 1e-8, ones_like_deta, zeros_like_deta)
        fwd_full_flow_deta_new = torch.where(torch.abs(fwd_full_flow_deta_old) > 1e-8, fwd_full_flow_deta_old, 1e-8 * ones_like_deta)
        bwd_full_flow_deta_new = torch.where(torch.abs(bwd_full_flow_deta_old) > 1e-8, bwd_full_flow_deta_old, 1e-8 * ones_like_deta)
        del ones_like_deta
        del zeros_like_deta
        ...
        fwd_mask_p01 = torch.nn.functional.unfold( mask_p01[:,1,:,:].unsqueeze(1) , (PATCH, PATCH)).permute(0,2,1)#[B, (H-2)*(W-2), 9]
###################################################
        fwd_mask_p01 = fwd_mask_p01 * fwd_mask_p01[:,:, int((PATCH * PATCH - 1) / 2)].unsqueeze(2)
        fwd_full_flow_numda_mu_p01_mask = Variable(fwd_full_flow_numda_mu_mask*fwd_mask_p01 , requires_grad = False) #[B, (H-2)*(W-2), 9]
        fwd_full_flow_intersect = torch.exp(-torch.abs(fwd_full_flow_numda - fwd_full_flow_mu ))* fwd_full_flow_numda_mu_p01_mask *  \
        ...
        #====================================================================================================================
        bwd_full_flow_numda = (-patches_coords_x_deta * flows_bwd_y_patches + patches_coords_y_deta * flows_bwd_x_patches) / bwd_full_flow_deta_new
        bwd_full_flow_mu = (patches_coords_y_deta * flows_bwd_x_patches_middle - patches_coords_x_deta * flows_bwd_y_patches_middle) / bwd_full_flow_deta_new

        bwd_full_flow_numda_mu_mask = ( ((bwd_full_flow_numda > 0) & (bwd_full_flow_numda < 1)) & \
	                 ((bwd_full_flow_mu > 0) & (bwd_full_flow_mu < 1)) ).float()
        bwd_full_flow_numda_mu_mask = Variable(bwd_full_flow_numda_mu_mask*mask_bwd_full_flow_deta,requires_grad = False)
        bwd_mask_p01 = torch.nn.functional.unfold( mask_p01[:,0,:,:].unsqueeze(1) ,( PATCH, PATCH)).permute(0,2,1)
        bwd_full_flow_numda_mu_p01_mask = Variable(bwd_full_flow_numda_mu_mask * bwd_mask_p01,requires_grad = False)
        bwd_full_flow_intersect = torch.exp(-torch.abs(bwd_full_flow_numda - bwd_full_flow_mu ))* bwd_full_flow_numda_mu_p01_mask *  \
                                           compute_intersect_loss_weights(tgt_img_scaled)#$$$$$$
        # print(bwd_full_flow_numda.size()) ,[B, (H-2)*(W-2), 9]


        del bwd_full_flow_numda_mu_mask
        del bwd_mask_p01
        del bwd_full_flow_numda
        del bwd_full_flow_mu
        
        

        loss = 0
        if float(fwd_full_flow_numda_mu_p01_mask.sum()) == 0 :
            loss += 1e-10
        else :
            # print(fwd_full_flow_intersect.sum())
            loss += weight/2 * (float(fwd_full_flow_intersect.sum()))/(float(fwd_full_flow_numda_mu_p01_mask.sum()))
        
        if float(bwd_full_flow_numda_mu_p01_mask.sum()) == 0:
            loss += 1e-10
        else :
            loss += weight/2 * (float(bwd_full_flow_intersect.sum()))/(float(bwd_full_flow_numda_mu_p01_mask.sum()))
