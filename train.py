...
import models
from utils import tensor2array, save_checkpoint, save_checkpoint_per_print_freq
from inverse_warp_summary import inverse_warp, pose2flow, flow2oob, flow_warp
...
parser = argparse.ArgumentParser(description='Unsupervised Learning of Depth, Optical Flow and Pose with Occlusion from 3D Geometry',
                                 formatter_class=argparse.ArgumentDefaultsHelpFormatter)
parser.add_argument('data', metavar='DIR',
                    help='path to dataset')
parser.add_argument('--kitti-dir', dest='kitti_dir', type=str, default='/home',
...
parser.add_argument('--spatial-normalize-max', dest='spatial_normalize_max', action='store_true', help='spatially normalize depth maps')
parser.add_argument('--spatial-normalize-mean', dest='spatial_normalize_mean', action='store_true', help='spatially normalize depth maps')
parser.add_argument('--spatial-normalize-min', dest='spatial_normalize_min', action='store_true', help='spatially normalize flow maps')
...
parser.add_argument('--flows-parameters', default='flows_parameters.csv', metavar='PATH',
                    help='csv where to save flows parammeters')
parser.add_argument('--depth-parameters', default='depth_parameters.csv', metavar='PATH',
                    help='csv where to save depth parammeters')                   
...
parser.add_argument('-w6', '--loss6-weight', type=float, help='weight for flow not intersect', metavar='W', default=0)
parser.add_argument('-w7', '--loss7-weight', type=float, help='weight for static consensus loss', metavar='W', default=0)
...
parser.add_argument('-f', '--training-output-freq', type=int, help='frequence for outputting dispnet outputs and warped imgs at training for all scales if 0 will not output',
                    metavar='N', default=0)
parser.add_argument('--using-none-mask', dest='using_none_mask', action='store_true', default=True, help='if using none as mask')
parser.add_argument('--add-maskp01', dest='add_maskp01', action='store_true', help='add-maskp01')
parser.add_argument('--add-occ-mask', dest='add_occ_mask', action='store_true', help='add-occ-mask')
parser.add_argument('--add-less-than-mean-mask', dest='add_less_than_mean_mask', action='store_true', help='add-less-than-mean-mask')
parser.add_argument('--use-disp', dest='use_disp', action='store_true', help='use-disp-in-smooth')
...
def main():
    ...
    if args.with_depth_gt:
        ...
        val_set = ValidationSet(
            args.data.replace('prepared_data_cs', 'prepared_data_completed'),
    ...
    with open(args.save_path/args.flows_parameters,'w') as csvfile:
        writer = csv.writer(csvfile, delimiter='\t')
        writer.writerow(['abs_diff', 'abs_rel', 'sq_rel', 'a1', 'a2', 'a3'])

    with open(args.save_path/args.depth_parameters,'w') as csvfile:
        writer = csv.writer(csvfile, delimiter='\t')
        writer.writerow(['epe_total', 'epe_rigid', 'epe_non_rigid', 'outliers', 'epe_total_with_gt_mask', 'epe_rigid_with_gt_mask', 'epe_non_rigid_with_gt_mask', 'outliers_gt_mask'])
    ...
    for epoch in range(args.epochs):
        ...
        if args.with_depth_gt:
            ...
            for error, name in zip(errors, error_names):
                ...
                out2csv.append(error.item())
            with open (args.save_path/args.flows_parameters,'a') as csvfile:
                writer = csv.writer(csvfile, delimiter='\t')
                writer.writerow(out2csv)
            del out2csv
        ...
        if args.with_flow_gt:
            ...
            for error, name in zip(flow_errors, flow_error_names):
                training_writer.add_scalar(name, error, epoch)
                out2csv.append(error)
            with open (args.save_path/args.depth_parameters,'a') as csvfile:
                writer = csv.writer(csvfile, delimiter='\t')
                writer.writerow(out2csv)
            del out2csv
        ...
        if not args.fix_posenet:
            decisive_error = flow_errors[-3]    # epe_rigid_with_gt_mask
        el
        """
        if not args.fix_dispnet:
            decisive_error = errors[1]      #depth abs_rel
        elif not args.fix_flownet:
            decisive_error = flow_errors[14]    #epe_non_rigid_with_gt_mask
...
def train(train_loader, disp_net, pose_net, mask_net, flow_net, optimizer, epoch_size, logger=None, train_writer=None):
    ...
    w6 = args.loss6_weight
    w7 = args.loss7_weight
    ...
    for i, (tgt_img, ref_imgs, intrinsics, intrinsics_inv) in enumerate(train_loader):
        ...
        for j in range(args.sequence_length - 1):
            if j == 0:
                dispnet_ref_imgs = ref_imgs_var[0]
            else:
                dispnet_ref_imgs = torch.cat([dispnet_ref_imgs, ref_imgs_var[j]], dim=0)
        # compute output
        disparities = disp_net(tgt_img_var)
        disparities_ref_imgs = disp_net(dispnet_ref_imgs)
        if args.spatial_normalize_max:
            disparities = [spatial_normalize_max(disp) for disp in disparities]
            disparities_ref_imgs = [spatial_normalize_max(disp) for disp in disparities_ref_imgs]
        if args.spatial_normalize_mean:
            disparities = [spatial_normalize_mean(disp) for disp in disparities]
            disparities_ref_imgs = [spatial_normalize_mean(disp) for disp in disparities_ref_imgs]
        depth = [1/disp for disp in disparities]
        depth_ref_imgs = [1/disp for disp in disparities_ref_imgs]
        ...
        flow_cam = pose2flow(depth[0].squeeze(), pose[:,1], intrinsics_var, intrinsics_inv_var)     # pose[:,1] belongs to forward frame
        ...
        exp_masks_target_fwd, exp_masks_target_bwd = consensus_exp_masks(flows_cam_fwd, flows_cam_bwd, flow_fwd, flow_bwd, tgt_img_var, ref_imgs_var[1], ref_imgs_var[0], wssim=args.wssim, wrig=args.wrig, ws=args.smooth_loss_weight )
        ...
        explainability_mask = a1_a2_rigid_mask(flows_cam_fwd, flow_fwd, flows_cam_bwd, flow_bwd)
        ...
        if args.no_non_rigid_mask:
            ...
            flow_exp_mask = [1 - exp_mask for exp_mask in explainability_mask]

        
        if w1 >0:
            if args.using_none_mask:

                loss_1 = loss_camera(tgt_img_var, ref_imgs_var, intrinsics_var, intrinsics_inv_var,
                                 depth, depth_ref_imgs, None, pose, lambda_oob=args.lambda_oob, qch=args.qch, wssim=args.wssim, 
                                 add_maskp01=args.add_maskp01, add_occ_mask=args.add_occ_mask, add_less_than_mean_mask = args.add_less_than_mean_mask)
            else :

                loss_1 = loss_camera(tgt_img_var, ref_imgs_var, intrinsics_var, intrinsics_inv_var,
                                 depth, depth_ref_imgs, explainability_mask_for_depth, pose, lambda_oob=args.lambda_oob, qch=args.qch, wssim=args.wssim, 
                                 add_maskp01=args.add_maskp01, add_occ_mask=args.add_occ_mask, add_less_than_mean_mask = args.add_less_than_mean_mask)
        else :
            loss_1 = 0
        ...
        if w3 >0 :
            ...
            if args.use_disp :
                loss_3 = edge_aware_smoothness_loss(tgt_img_var, disparities) 

            else :
                loss_3 = edge_aware_smoothness_loss(tgt_img_var, depth)
            
            if args.spatial_normalize_min:
                loss_3 += edge_aware_smoothness_loss(tgt_img_var, [spatial_normalize_min(flow) for flow in flow_fwd]) 
                loss_3 += edge_aware_smoothness_loss(tgt_img_var, [spatial_normalize_min(flow) for flow in flow_bwd]) 
            else:
                loss_3 += edge_aware_smoothness_loss(tgt_img_var, flow_fwd) 
                loss_3 += edge_aware_smoothness_loss(tgt_img_var, flow_bwd) 

        else :
            loss_3 = 0

        if w4 > 0:
            maskp01_for_flow, maskp01_for_visual = maskp01_mask(flows_cam_fwd, flow_fwd, flows_cam_bwd, flow_bwd,   tgt_img_var, ref_imgs_var, intrinsics_var, intrinsics_inv_var, depth, depth_ref_imgs, pose)
            if args.using_none_mask :
                loss_4 = loss_flow(maskp01_for_flow, tgt_img_var, ref_imgs_var, [flow_bwd, flow_fwd], intrinsics_var, intrinsics_inv_var, depth, depth_ref_imgs, None, pose,
                                lambda_oob=args.lambda_oob, qch=args.qch, wssim=args.wssim,
                                add_maskp01=args.add_maskp01, add_occ_mask=args.add_occ_mask, add_less_than_mean_mask = args.add_less_than_mean_mask)
            else :
                loss_4 = loss_flow(maskp01_for_flow, tgt_img_var, ref_imgs_var, [flow_bwd, flow_fwd], intrinsics_var, intrinsics_inv_var, depth, depth_ref_imgs, flow_exp_mask, pose,
                                lambda_oob=args.lambda_oob, qch=args.qch, wssim=args.wssim,
                                add_maskp01=args.add_maskp01, add_occ_mask=args.add_occ_mask, add_less_than_mean_mask = args.add_less_than_mean_mask)
        ...
        loss_5 = consensus_depth_flow_mask(explainability_mask, rigidity_mask_bwd, rigidity_mask_fwd,
                                        exp_masks_target_bwd, exp_masks_target_fwd, THRESH=args.THRESH, wbce=args.wbce)

        if w6>0:
            loss_6, flow_numda_mu_p01_mask_fwd, flow_intersect_fwd, flow_numda_mu_p01_mask_bwd, flow_intersect_bwd = build_flow_not_intersect(maskp01_for_flow, [flow_bwd, flow_fwd], tgt_img_var, ref_imgs_var,intrinsics_var, intrinsics_inv_var, depth, depth_ref_imgs,pose)
        ...
        if w7>0:           
            ...
            loss_7 = depth_supervise_flow(flows_cam_fwd, flow_fwd, flows_cam_bwd, flow_bwd, maskp01 = maskp01_for_flow, add_a1_a2_mask = False)
        ...
        if args.training_output_freq > 0 and n_iter % args.training_output_freq == 0:
            ...
            #-------------------------------------------------------------------------------------------------------------------------------------
            mean_depth_mask = less_than_mean_depth_mask(tgt_img_var, ref_imgs_var, intrinsics_var, intrinsics_inv_var,
                                        depth, explainability_mask_for_depth, pose, lambda_oob=args.lambda_oob, qch=args.qch, wssim=args.wssim)
            # print(mean_depth_mask.size())
            mean_depth_mask = mean_depth_mask[:,0,:,:].unsqueeze(1)
            mean_flow_mask = less_than_mean_flow_mask(tgt_img_var, ref_imgs_var, [flow_bwd, flow_fwd], flow_exp_mask,
                                        lambda_oob=args.lambda_oob, qch=args.qch, wssim=args.wssim)
            mean_flow_mask = mean_flow_mask[:,0,:,:].unsqueeze(1)
            train_writer.add_image('less than mean mask for flow', tensor2array(mean_flow_mask.data[0].cpu(), max_value=1, colormap='bone'), n_iter)
            train_writer.add_image('less than mean mask for depth', tensor2array(mean_depth_mask.data[0].cpu(), max_value=1, colormap='bone'), n_iter)

            if w4 >0:
                mask_p01 =  maskp01_for_visual[0]                 
                train_writer.add_image('mask_p01-bwd', tensor2array(mask_p01[:,0,:,:].unsqueeze(1).data[0].cpu(), max_value=1, colormap='bone'), n_iter)
                train_writer.add_image('mask_p01-fwd', tensor2array(mask_p01[:,1,:,:].unsqueeze(1).data[0].cpu(), max_value=1, colormap='bone'), n_iter)
                train_writer.add_image('mask_p1-bwd', tensor2array(mask_p01[:,2,:,:].unsqueeze(1).data[0].cpu(), max_value=1, colormap='bone'), n_iter)
                train_writer.add_image('mask_p1-fwd', tensor2array(mask_p01[:,3,:,:].unsqueeze(1).data[0].cpu(), max_value=1, colormap='bone'), n_iter)
                
                train_writer.add_image('mask_p-bwd', tensor2array(mask_p01[:,4,:,:].unsqueeze(1).data[0].cpu(), max_value=1, colormap='bone'), n_iter)
                train_writer.add_image('mask_p-fwd', tensor2array(mask_p01[:,5,:,:].unsqueeze(1).data[0].cpu(), max_value=1, colormap='bone'), n_iter)
                train_writer.add_image('mask_0-bwd', tensor2array(mask_p01[:,6,:,:].unsqueeze(1).data[0].cpu(), max_value=1, colormap='bone'), n_iter)
                train_writer.add_image('mask_0-fwd', tensor2array(mask_p01[:,7,:,:].unsqueeze(1).data[0].cpu(), max_value=1, colormap='bone'), n_iter)

                train_writer.add_image('mask_1-bwd', tensor2array(mask_p01[:,8,:,:].unsqueeze(1).data[0].cpu(), max_value=1, colormap='bone'), n_iter)
                train_writer.add_image('mask_1-fwd', tensor2array(mask_p01[:,9,:,:].unsqueeze(1).data[0].cpu(), max_value=1, colormap='bone'), n_iter)
                
                del mask_p01
                del maskp01_for_visual

            if w6>0:
                train_writer.add_image('flow_numda_mu_p01_mask_fwd', tensor2array(flow_numda_mu_p01_mask_fwd.data[0].cpu(), max_value=1, colormap='bone'), n_iter)
                train_writer.add_image('flow_intersect_fwd', tensor2array(flow_intersect_fwd.data[0].cpu(), max_value=1, colormap='bone'), n_iter)
                train_writer.add_image('flow_numda_mu_p01_mask_bwd', tensor2array(flow_numda_mu_p01_mask_bwd.data[0].cpu(), max_value=1, colormap='bone'), n_iter)
                train_writer.add_image('flow_intersect_bwd', tensor2array(flow_intersect_bwd.data[0].cpu(), max_value=1, colormap='bone'), n_iter)
            ...
            for k,scaled_depth in enumerate(depth):
                ...
                train_writer.add_image('train Non Rigid Warped Image {}'.format(k),
                                        tensor2array(flow_warp(ref_imgs_scaled[1],flow_fwd[k]).data[0].cpu()) , n_iter )
...
def validate_depth_with_gt(val_loader, disp_net, epoch, logger, output_writers=[]):
    ...
    for i, (tgt_img, depth) in enumerate(val_loader):
        ...
        if args.spatial_normalize_max:
            output_disp = spatial_normalize_max(output_disp)
        if args.spatial_normalize_mean:
            output_disp = spatial_normalize_mean(output_disp)
...
def validate_flow_with_gt(val_loader, disp_net, pose_net, mask_net, flow_net, epoch, logger, output_writers=[]):
    ...
    error_names = ['epe_total', 'epe_rigid', 'epe_non_rigid', 'outliers', 'epe_total_with_gt_mask', 'epe_rigid_with_gt_mask', 'epe_non_rigid_with_gt_mask', 'outliers_gt_mask',
                    'epe_total_all_1', 'epe_rigid_all_1', 'epe_non_rigid_all_1', 'outliers_all_1', 'epe_total_all_0', 'epe_rigid_all_0', 'epe_non_rigid_all_0', 'outliers_all_0']
    errors = AverageMeter(i=len(error_names))
    log_outputs = len(output_writers) > 0
    ...
    for i, (tgt_img, ref_imgs, intrinsics, intrinsics_inv, flow_gt, obj_map_gt) in enumerate(val_loader):
        with torch.no_grad():
            ...
            flow_gt_var = flow_gt.cuda()
            obj_map_gt_var = obj_map_gt.cuda()

        # compute output
        disp = disp_net(tgt_img_var)
        if args.spatial_normalize_max:
            disp = spatial_normalize_max(disp)
        if args.spatial_normalize_mean:
            disp = spatial_normalize_mean(disp)
        ...
        rigidity_mask_fwd = consensus_exp_masks(flows_cam_fwd, flows_cam_bwd, flow_fwd_list, flow_bwd_list, tgt_img_var, ref_imgs_var[1], ref_imgs_var[0], wssim=args.wssim, wrig=args.wrig, ws=args.smooth_loss_weight )[0]
        del flow_fwd_list
        del flow_bwd_list
        #--------------------------------------------------------------
        ...
        rigidity_mask_combined = 1 - (1-rigidity_mask_fwd[0])*(1-rigidity_mask_census) 
        ...
        if log_outputs and i % 10 == 0 and i/10 < len(output_writers):
            ...
            output_writers[index].add_image('（14） Total Output', flow_to_image(tensor2array(total_flow.data[0].cpu())), epoch)
            output_writers[index].add_image('（14） Rigid  Output', flow_to_image(tensor2array(flow_fwd_rigid.data[0].cpu())), epoch)
            output_writers[index].add_image('（14） Non-rigid  Output', flow_to_image(tensor2array(flow_fwd_non_rigid.data[0].cpu())), epoch)
            ...
            output_writers[index].add_image('（12） Rigidity Mask_left', tensor2array(rigidity_mask_fwd[0].data[0].cpu(), max_value=1, colormap='bone'), epoch)
            output_writers[index].add_image('（12） Rigidity Mask_right', tensor2array(rigidity_mask_census.data[0].cpu(), max_value=1, colormap='bone'), epoch)
            ...
            for j,ref in enumerate(ref_imgs_var):
                ...
                if rigidity_mask_fwd is not None:
                    output_writers[index].add_image('val Exp mask Outputs {}'.format(j), tensor2array(rigidity_mask_fwd[0].data.cpu(), max_value=1, colormap='bone'), epoch)
        ...
        _epe_errors = compute_all_epes(flow_gt_var, flow_cam, flow_fwd, rigidity_mask_combined) + compute_all_epes(flow_gt_var, flow_cam, flow_fwd, (1-obj_map_gt_var_expanded) ) \
                            + compute_all_epes(flow_gt_var, flow_cam, flow_fwd, torch.ones_like(rigidity_mask_combined)) + compute_all_epes(flow_gt_var, flow_cam, flow_fwd, torch.zeros_like(rigidity_mask_combined)) 
